...
func add_pool{
    ...
    pedersen_ptr: HashBuiltin*,
    range_check_ptr
}(token_in: felt, token_out: felt, pool_address: felt):
  let (token_1: felt, token_2: felt) = sort_tokens(token_in, token_out)

  with_attr error_message("Token 1 must be lt Token 2"):
    assert_lt(token_1, token_2) 
  end
...
#   _pool_addres.write(pool_address, token_1, token_2)
  return ()
end
...
@view
func sort_tokens_test{
    syscall_ptr: felt*,
    pedersen_ptr: HashBuiltin*,
    range_check_ptr
}(token_in: felt, token_out: felt) -> (in_lt_out: felt):
    assert_not_zero(token_out)

    # a < b == a <= (b-1)
    let (in_lt_out: felt) = is_le(token_in, token_out)
    return (in_lt_out)
end
...
#     pedersen_ptr: HashBuiltin*,
#     range_check_ptr
# }(pool_address: felt) -> (token_in: felt, token_out: felt):
#     alloc_locals
#     let(token_in: felt, token_out: felt) = _pool_addres.read(pool_address)
#     return (token_in, token_out)
# end
