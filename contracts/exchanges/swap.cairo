...
namespace Swap_Trade {
    ...
    func swap_item{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        IERC721.transferFrom(
            swapTrade.target_token_contract, caller, swapTrade.owner, swapTrade.target_token_id
        );
    ...
    func accept_bid{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        // İf seller wants nft + eth
        transfer_currency(
            swapTrade.owner, swapBid.bid_owner, swapBid.price, swapBid.currency_address
        );
        // transfer item to seller from bidder
        IERC721.transferFrom(
            swapTrade.token_contract, swapTrade.owner, swapBid.bid_owner, swapTrade.token_id
        );
        // # transfer item to bidder from seller
        IERC721.transferFrom(
            swapBid.bid_contract_address, swapBid.bid_owner, caller, swapBid.bid_token_id
        );
    ...
    // Returns all bid according to given trade_id
    @view
    func get_all_bids{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr: felt}(
        trade_id: felt
    ) -> (bids_ptr_len: felt, bids_ptr: SwapBid*) {
        alloc_locals;
        let (bids_count) = Swap_Trade.get_bid_count(trade_id);

        let (local bids_ptr: SwapBid*) = alloc();

        get_bids(trade_id=trade_id, bids_count=bids_count, bids_ptr_len=0, bids_ptr=bids_ptr);

        return (bids_count, bids_ptr);
    }
    
    func get_bids{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr: felt}(
        ...
        let (bid) = Swap_Trade.bid(trade_id, bids_ptr_len + 1);
...
func assert_swap_buyer_seller{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (buyer_item_owner) = IERC721.ownerOf(
        _swap_trade.target_token_contract, _swap_trade.target_token_id
    );
    assert seller_item_owner = _swap_trade.owner;
    assert buyer_item_owner = caller;

    let (seller_approve) = IERC721.isApprovedForAll(
        _swap_trade.token_contract, _swap_trade.owner, contract_address
    );
    let (buyer_approve) = IERC721.isApprovedForAll(
        _swap_trade.target_token_contract, caller, contract_address
    );
